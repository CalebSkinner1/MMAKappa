...
<!-- *Is there a way to highlight values along the diagonal in a contingency table?* -->
...
*in isle, we'll allow these tables to scroll*
...
Surprisingly, D'Amato and Lee's simple agreement (proportion of observed agreement) is only `r percent(DL_obs_conc_rate, accuracy = .01)`. This means they disagreed on almost 1/5 of their rulings.
...
Correct Answer: `r pa_answers_dc`

How do the results compare with D'Amato and Lee?

\leavevmode \newline

As before, D'Amato is more inclined to vote for a draw, but generally, Cleary and D'Amato agree more than D'Amato and Lee.
...
In the ISLE version, you would be able to select any two judges to compare below.
...
In the pdf version, we have given a third example using the top judge combination Cartlidge and Lethaby.
...
Correct Answer: `r pa_answers_jj`

How do `r stu_picks[1]` and `r stu_picks[2]`'s results compare to the other judges we assessed?
...
The proportion of observed agreement for D'Amato and Cleary is `r DC_obs_conc_perc` and the proportion of expected agreement for D'Amato and Cleary is `r DC_exp_conc_perc`.

After some calculations, we find that D'Amato and Cleary's kappa is `r round(kappa_DC, 2)`. Using the guidelines demonstrated above, interpret the value.
...
In the ISLE format, we would give you the opportunity to select any of the judge combinations to compare. However, for the pdf format, we have selected Cleary and Lee.

Assess the relationship between Cleary and Lee. How does it compare to the relationships we have seen before? How large is the sample between the two judges? The lower the sample size, the wider our confidence interval and the less we can trust our kappa value.
...
The proportion of observed agreement for `r stu_picks[1]` and `r stu_picks[2]` is `r DC_obs_conc_perc` and the proportion of expected agreement for `r stu_picks[1]` and `r stu_picks[2]` is `r DC_exp_conc_perc`.

After solving for kappa, our value is `r round(kappa_DC, 2)`. Using the guidelines demonstrated previously, interpret the value. Does it surprise you? How does it compare to previous judge-pairings kappas?
...
Interpret the confidence interval and p-value. What does they mean for the relationship between the two judges?

\leavevmode \newline

If you were to assess the strength of the judges' relationships, which factor would you emphasize?
...
Take a second to look through the contingency table. Trace your eyes along the diagonal of concordant values. How often do the judges completely agree? How often do they agree in their verdict (win/lose/draw), but disagree in the margin? What are the most common frequencies? Does this make sense contextually?
...
DLw_exp %>%
  ...
  # kable_styling(latex_options = "HOLD_position")
```
What values are the largest? Is this surprising? Does the observed agreement surpass the expected agreement? By how much?
...
What do you notice about D'Amato and Cleary? Compare them to D'Amato and Lee. How often do the judges completely agree? How often do they agree in their verdict (win/lose/draw), but disagree in the margin? What are the most common scoring margins? Does this make sense contextually?
...
Look through the expected value table. Which values are highly expected? Did they occur frequently in the observed table? Based on the two tables, would you expect there to be a large association?
...
Use the above information to assess the consistency of D'Amato and Cleary. What is their reproducibility? How confident are you in the interchangeability of D'Amato and Cleary? How much of the data is represented by chance and disagreement?

\leavevmode \newline

Assess the confidence interval. Can you reject our null hypothesis?
...
Use the above information to assess the consistency of D'Amato and Cleary. What is the reproducibility? How confident are you in the interchangeability of D'Amato and Cleary? How much of the data is represented by chance and disagreement?
...
How large is the difference between the quadratic and linear weights? Which do you feel is more accurate? Pick one and argue for it.

\leavevmode \newline

## Other Judges

Once again, in the ISLE version, we'd give you the opportunity to select your own judge pairing. For this pdf, we'll give you an extra example analyzing D'Amato and Kamijo.
...
What do you notice about the two judges? Compare them to some of the other pairs you've looked at. How often do the judges completely agree? How often do they agree in their verdict (win/lose/draw), but disagree in the margin? What are the most common scoring margins? Does this make sense contextually?
...
Use the above information to assess the consistency of `r stu_picks[1]`, `r stu_picks[2]`, and `r stu_picks[3]`. What is the reproducibility? How confident are you in the interchangeability of the three? How much of the data is represented by chance and disagreement?
